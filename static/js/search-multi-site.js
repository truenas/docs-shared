...
const searchConfig = {
  indexes: {
    ...
    connect: {
      ...
      iconSvg: '<svg style="width: 20px; height: 20px;" viewBox="0 0 422.27 96.59"><polygon points="0 0 0 96.59 374.87 96.59 422.27 47.85 374.87 0 0 0"/></svg>',
      priority: 13,
      group: 'other'
    }
  }
...
class MultiSiteSearch {
  ...
  detectDefaultVersion() {
    // Check site config first (set by Hugo template from software_status_config.yaml)
    const siteConfig = window.SITE_SEARCH_CONFIG || {};
    if (siteConfig.defaultVersion) {
      return siteConfig.defaultVersion;
    }

    // Fall back to URL detection
    const urlVersion = this.detectCurrentDocsVersionFromUrl();
    if (urlVersion) {
      return urlVersion;
    }

    // Fall back to YAML data (set during initialization)
    if (this.currentVersionFromYaml) {
      return this.currentVersionFromYaml;
    }
  ...
  setDefaultVersionCheckbox() {
    // Determine which version to select (checks site config, then URL, then YAML)
    let versionToSelect = this.detectDefaultVersion();
  ...
  async init() {
    ...
    const trigger = document.getElementById('keyword-search-trigger');
    const closeBtn = document.querySelector('.enhanced-search-close');
    const overlay = document.getElementById('enhanced-search-overlay');
    const submitBtn = document.getElementById('enhanced-search-submit');
    ...
    if (overlay) {
      overlay.addEventListener('click', (e) => {
        // Only close if clicking directly on the overlay (dark background), not the modal content
        if (e.target === overlay) {
          this.closeModal();
        }
      });
    ...
    if (docsVersionCheckboxes && docsMainToggle) {
      docsVersionCheckboxes.forEach(checkbox => {
        checkbox.addEventListener('change', () => {
          this.updateDocsToggle();
          // If there's a search query or results are displayed, automatically re-search
          const searchInput = document.getElementById('enhanced-search-input');
          if (this.hasResults || (searchInput && searchInput.value.trim())) {
            this.performSearch();
          }
        });
      });
    ...
    document.addEventListener('keydown', (e) => {
      ...
      if (e.key === 'Escape') {
        const modalOverlay = document.getElementById('enhanced-search-overlay');
  ...
  toggleSiteCheckbox(checkbox) {
    ...
    // If there's a search query or results are displayed, automatically re-search
    const searchInput = document.getElementById('enhanced-search-input');
    if ((this.hasResults || (searchInput && searchInput.value.trim())) && this.activeSites.length > 0) {
      this.performSearch();
    }
  }
  ...
  openModal() {
    const modal = document.getElementById('enhanced-search-overlay');
    if (modal) {
  ...
  async performSearch() {
    ...
    const countDiv = document.getElementById('enhanced-search-count');
  ...
  displayResults() {
    const container = document.getElementById('enhanced-search-results');
    const countContainer = document.getElementById('enhanced-search-count');
...
function openEnhancedSearch() {
  if (window.multiSiteSearch) {
    window.multiSiteSearch.openModal();
  }
}

function closeEnhancedSearch() {
  if (window.multiSiteSearch) {
    window.multiSiteSearch.closeModal();
  }
}
